      ...
      <Paper elevation={0} sx={{ border: "1px solid #e3f2fd", borderRadius: 3, overflow: "hidden" }}>
        ...
        <Box sx={{ px: 4, py: 4 }}>
          ...
          <Stack spacing={3}>
            ...
            <Box>
              <Typography variant="subtitle2" fontWeight={600} gutterBottom sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                {muted ? <VolumeOffIcon fontSize="small" sx={{ color: "text.secondary" }} /> : <VolumeUpIcon fontSize="small" sx={{ color: "primary.main" }} />}
                Sound
              </Typography>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setAvatarFile(null)
        setSuccess("Settings saved successfully!")
        // Update username state in case it changed
        setUsername(json.user.username)
      }
    } catch {
      ...
      <Paper elevation={0} sx={{ border: "1px solid #e3f2fd", borderRadius: 3, overflow: "hidden" }}>
        ...
        <Box sx={{ bgcolor: "#f0f7ff", px: 4, py: 4, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
          <Box sx={{ position: "relative", display: "inline-flex" }}>
            <Avatar
              src={avatarSrc}
              sx={{ width: 100, height: 100, fontSize: 40, bgcolor: "primary.main", border: "3px solid #fff", boxShadow: 2 }}
            >
              {!avatarSrc && user.username[0].toUpperCase()}
            </Avatar>
            <Box
          ...
          <Stack spacing={3}>
            <Box>
              ...
              <TextField
                ...
                helperText="This also changes your profile URL (/user/username)"
              />
            </Box>
            ...
            <Box>
              ...
              <Box
                ...
                sx={{
                  ...
                  textAlign: "center",
                  cursor: "pointer",
                  bgcolor: avatarFile ? "#f0f7ff" : "#fafafa",
                ...
                {avatarFile ? (
                  <Box>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      {(avatarFile.size / 1024).toFixed(0)} KB — Click to change
                    </Typography>
                  </Box>
                ) : (
                  <Box>
            ...
            <Button
              ...
              onClick={handleSave}
              disabled={saving}
              sx={{ fontWeight: 700, display: "flex", gap: 1, alignItems: "center" }}
            >
              {saving && <CircularProgress size={16} sx={{ color: "inherit" }} />}
              {saving ? "Saving..." : "Save Changes"}
            </Button>
